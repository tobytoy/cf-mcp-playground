...
/**
 * Safe, zero-hallucination mathematical calculator.
 * Evaluates expressions using a recursive descent parser for complete AST safety without eval().
 */
export function evaluateMathExpression(rawExpr: string): CalculationResult {
  ...
  try {
    ...
    // Format output cleanly (handling floating point quirks e.g. 0.1 + 0.2 = 0.30000000000000004)
    let formattedResult: string | number = value;
    if (typeof value === "number") {
      if (Number.isInteger(value)) {
        formattedResult = value.toLocaleString("en-US");
      } else {
        // Round to max 10 decimal digits
        formattedResult = Number(value.toFixed(10)).toString();
      }
    }
    ...
    return {
      ...
      explanation: generateExplanation(cleanExpr, formattedResult),
      success: true
    };
  } catch (err: unknown) {
...
function generateExplanation(expr: string, result: string | number): string {
  if (expr.includes("^")) {
    return "已執行指數/複利冪次方運算。";
  }
  if (expr.includes("%")) {
    return "已執行百分比折算運算。";
  }
